...
unsigned long get_time() {
	...
	ret = usec/1000;
	ret = ret + tv.tv_sec*1000;//+tv.tv_usec;
	return usec;
}
...
*/
   ...
   printf("ID=\r\n");  
	...
	else
	{
       ...
       ADS1256_StartScan(0);
	ch_num = 8; //Changed from 8 to 3 for this application //
	//if (ADS1256_Scan() == 0)
		//{
			//continue;
		//}

	//Opening File for data collection
	...
	snprintf(buffer, sizeof(char)*100,"/home/pi/Documents/DataCollection/%s",filename);
	printf("%s\n",buffer);

	Datafile = fopen(buffer,"w+");

	time_start = (double)time(NULL);
	while(((double)time(NULL)-time_start) < Duration)
	{
		for (i = 0; i < 3; i = i + 1) {
		...
		if (Count % 8 == 0) {
			...
			if ((volt[0] < 0) || (volt[1] < 0) || (volt[2] < 0)) {
				volt[0] = -volt[0];
				volt[1] = -volt[1];
				volt[2] = -volt[2];
//				printf("-%ld.%03ld%03ld \r\n",volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000);
//				printf("-%ld.%03ld%03ld \r\n",volt[1]/1000000,(volt[1]%1000000)/1000,volt[1]%1000);
//				printf("-%ld.%03ld%03ld \r\n",volt[2]/1000000,(volt[2]%1000000)/1000,volt[2]%1000);
//				printf("-%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld  \r\n",volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000,volt[1]/1000000,(volt[1]%1000000)/1000,volt[1]%1000,volt[2]/1000000,(volt[2]%1000000)/1000,volt[2]&1000);
//				printf("\n");
//                        	fprintf(Datafile ,"%lu.%lu -%ld.%03ld%03ld \r\n",Times,Timeus,volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000);
				if (Timeus < 100000) {
					fprintf(Datafile ,"%lu.0%lu -%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld \r\n",Times,Timeus,volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000,volt[1]/1000000,(volt[1]%1000000)/1000,volt[1]%1000,volt[2]%1000000,(volt[2]%1000000)/1000,volt[2]%1000);
				}
				else {
					fprintf(Datafile ,"%lu.%lu -%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld \r\n",Times,Timeus,volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000,volt[1]/1000000,(volt[1]%1000000)/1000,volt[1]%1000);
				}
			}
			else {
//				printf("-%ld.%03ld%03ld \r\n",volt[0]/1000000,(volt[0]%1000000)/1000,volt[0]%1000);
