...
void WriteToFile(long unsigned int * TimeSec, long unsigned int * TimeUS, uint32_t * ch1, uint32_t * ch2, uint32_t * ch3, char * filename, int Num,int SampleSize);
...
void WriteToFile(long unsigned int * TimeSec, long unsigned int * TimeUS, uint32_t * ch1, uint32_t * ch2, uint32_t * ch3, char * filename, int Num, int SampleSize) {
	...
	snprintf(buffer, sizeof(char)*100,"/home/pi/Documents/DataCollection/%s%d.txt",filename,Num);
        printf("%s\n",buffer);

        Datafile = fopen(buffer,"w+");
	for (i = 0; i < length; ++i)  {

	    if ((ch1[i] < 0) || (ch2[i] < 0) || (ch3[i] < 0)) {
            	ch1[i] = -ch1[i];
            	ch2[i] = -ch2[i];
            	ch3[i] = -ch3[i];
            	printf("-%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld  \r\n",ch1[i]/1000000,(ch1[i]%1000000)/1000,ch1[i]%1000,ch2[i]/1000000,(ch2[i]%1000000)/1000,ch2[i]%1000,ch3[i]/1000000,(ch3[i]%1000000)/1000,ch3[i]%1000);
      		printf("\n");
                	if (TimeUS[i] < 100000) {
                    	fprintf(Datafile,"%lu.0%lu -%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld \r\n",TimeSec[i],TimeUS[i],ch1[i]/1000000,(ch1[i]%1000000)/1000,ch1[i]%1000,ch2[i]/1000000,(ch2[i]%1000000)/1000,ch2[i]%1000,ch3[i]/1000000,(ch3[i]%1000000)/1000,ch3[i]%1000);
                	}
                	else {
                    	fprintf(Datafile,"%lu.0%lu -%ld.%03ld%03ld -%ld.%03ld%03ld -%ld.%03ld%03ld \r\n",TimeSec[i],TimeUS[i],ch1[i]/1000000,(ch1[i]%1000000)/1000,ch1[i]%1000,ch2[i]/1000000,(ch2[i]%1000000)/1000,ch2[i]%1000,ch3[i]/1000000,(ch3[i]%1000000)/1000,ch3[i]%1000);
               		}
			...
			if (TimeUS[i] < 100000) {
                	    fprintf(Datafile,"%lu.0%lu %ld.%03ld%03ld %ld.%03ld%03ld %ld.%03ld%03ld \r\n",TimeSec[i],TimeUS[i],ch1[i]/1000000,(ch1[i]%1000000)/1000,ch1[i]%1000,ch2[i]/1000000,(ch2[i]%1000000)/1000,ch2[i]%1000,ch3[i]/1000000,(ch3[i]%1000000)/1000,ch3[i]%1000);
                    	    ...
                    	    fprintf(Datafile,"%lu.%lu %ld.%03ld%03ld %ld.%03ld%03ld %ld.%03ld%03ld \r\n",TimeSec[i],TimeUS[i],ch1[i]/1000000,(ch1[i]%1000000)/1000,ch1[i]%1000,ch2[i]/1000000,(ch2[i]%1000000)/1000,ch2[i]%1000,ch3[i]/1000000,(ch3[i]%1000000)/1000,ch3[i]%1000);
                	}
            }
        }
	fclose(Datafile);
...
int  main()
{
	...
	uint32_t Cha2[1000];
	uint32_t Cha3[1000];
	unsigned long TimeSec[1000];
	unsigned long TimeUS[1000];
...
*/
   ...
   printf("ID=\r\n");  
	...
	printf("Enter File Name (No Extension): ");
        scanf("%s",&filename);
//	filename = "Test";
//        printf("Number of Samples per File: ");
//        scanf("%d",&SampleNumber);
	SampleNumber = 1000; //Remember to change memory
	printf("Enter time duration of the measurement in seconds: ");
        scanf("%d",&Duration);
//	Duration = 3600;
	...
	while ( Flag == 0)
	{
		...
		if (LoopCount % 8 == 0) {
			struct timeval tv;
        	        ...
        	        unsigned long Timeus = tv.tv_usec;
	                unsigned long Times = tv.tv_sec;
		    if (LoopCount / 8 == SampleNumber ) {
			    WriteToFile(TimeSec,TimeUS,Cha1,Cha2,Cha3,filename,FileCount,SampleNumber);
			    FileCount = ++FileCount;
			    LoopCount = 0;
			    DataCount = 0;
				if (((double)time(NULL)-time_start) > Duration) {
					Flag = 1;
				}
		    }
		    else {
//			if (volt[2] < 0) || (volt[1]  {
//				volt[2]=-volt[2];
//				printf("-%ld.%03ld%03ld \r\n",volt[2]/1000000,(volt[2]%1000000)/1000,volt[2]%1000);
//			}
//			else {
//				printf("%ld.%03ld%03ld \r\n",volt[2]/1000000,(volt[2]%1000000)/1000,volt[2]%1000);
//			}
                	    TimeSec[DataCount] = tv.tv_sec;
                	    TimeUS[DataCount] = tv.tv_usec;
                	    Cha1[DataCount] = volt[0];
                	    Cha2[DataCount] = volt[1];
                	    Cha3[DataCount] = volt[2];
			    FlagReturn = 0;
		    }
		    DataCount = ++DataCount;
        	}
		bsp_DelayUS(100); /* Changes the amount of bits sent per second change to 100000 for 0.1s between data sent */
		LoopCount = ++LoopCount;
	}
